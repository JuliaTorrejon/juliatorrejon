...
- Find specofic data in a structure (for example, a substring within a string)
...
- **O(log n)**: A typical example of this kind of operation is finding a specific value in a sorted array using  binary search, so as the number of items in the sorted array grows, it only takes a logarithmin time relationship to find any given item. 
